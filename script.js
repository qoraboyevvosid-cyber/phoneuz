// ===== SOZLAMALAR =====
const TELEGRAM_TOKEN = '8462199964:AAHzm0S7ni0nBuzrOHqVxoz2kfekjpUKa3A';
...
function loadProducts() {
  try {
    const saved = localStorage.getItem('phoneuz_products');
...
function saveProducts() {
  try {
    localStorage.setItem('phoneuz_products', JSON.stringify(products));
  } catch(e) {
...
function loadOrders() {
  try {
    const saved = localStorage.getItem('phoneuz_orders');
...
function saveOrders() {
  try {
    localStorage.setItem('phoneuz_orders', JSON.stringify(orders));
  } catch(e) {}
}
  ...
  filteredProducts.forEach((p, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="product-body">
        <div>${p.isNew ? '<span class="badge-new">YANGI</span>' : ''}</div>
...
function applyDiscount() {
  ...
  if (DISCOUNT_CODES[code]) {
    ...
    msgEl.innerHTML = `<span class="discount-ok">✅ ${appliedDiscount}% chegirma!</span>`;
    updateCartUI();
...
function deleteProduct(id) {
  ...
  saveProducts(); // localStorage ga saqlash
  renderProducts();
  renderAdminProducts();
...
// Mahsulotlarni default ga qaytarish
function resetProducts() {
  if (!confirm('Barcha mahsulotlarni asl holatga qaytarasizmi?')) return;
